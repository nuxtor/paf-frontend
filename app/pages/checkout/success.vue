...
definePageMeta({
  // The order is placed by the time anyone sees this, so the customer is out
  // of checkout and back on the site proper — the stripped-down checkout chrome
  // (no nav, a cut-down footer) is for the pages where a distraction costs a
  // sale. This one gets the same header and footer as everywhere else.
  layout: 'default',
})
...
<template>
  <div class="container py-16">
    ...
    <div v-if="isLoading" class="max-w-lg mx-auto text-center">
      ...
      <p class="text-gray-500 dark:text-gray-400">Confirming your order...</p>
    </div>
    ...
    <div v-else-if="error" class="max-w-lg mx-auto text-center">
      <div class="w-20 h-20 bg-red-100 dark:bg-red-900/20 rounded-full flex items-center justify-center mx-auto mb-6">
        <Icon name="heroicons:exclamation-triangle" class="w-10 h-10 text-red-600 dark:text-red-400" />
      </div>
      <h1 class="font-heading text-3xl text-pif-black dark:text-white mb-4">Something Went Wrong</h1>
      ...
      <div class="space-y-4">
        ...
        <NuxtLink
          to="/"
          class="block w-full py-3 border border-pif-green-dark text-pif-green-dark dark:text-pif-gold dark:border-pif-gold font-medium rounded-lg hover:bg-pif-green-dark hover:text-white dark:hover:bg-pif-gold dark:hover:text-pif-black transition-colors text-center"
        >
          Go Home
        </NuxtLink>
      </div>
    ...
    <div v-else-if="order" class="max-w-lg mx-auto text-center">
      ...
      <h1 class="font-heading text-3xl text-pif-black dark:text-white mb-4">Thank You for Your Order!</h1>

      <p class="text-gray-600 dark:text-gray-400 mb-8">
        Your order has been placed successfully. We've sent a confirmation email with all the details.
      </p>

      <div class="bg-gray-50 dark:bg-dark-100 rounded-xl p-6 mb-8">
        <p class="text-sm text-gray-500 dark:text-gray-400 mb-2">Order Number</p>
        <p class="text-xl font-semibold text-pif-black dark:text-white">#{{ order.order_number }}</p>
        <div class="mt-4 flex items-center justify-center gap-4 text-sm text-gray-500 dark:text-gray-400">
          <span>Status: <span class="font-medium text-pif-green-dark dark:text-green-400 capitalize">{{ order.status }}</span></span>
          <span>Total: <span class="font-medium text-pif-green-dark dark:text-green-400">{{ formatCurrency(toAmount(order.total)) }}</span></span>
        </div>
      </div>
      ...
      <div
        ...
        class="bg-white dark:bg-dark-200 rounded-xl border border-gray-200 dark:border-dark-600 p-4 mb-8 text-left"
      >
        <h3 class="font-medium text-pif-black dark:text-white mb-3">Items Ordered</h3>
        <div class="space-y-2">
          <div
          ...
          >
            <span class="text-gray-700 dark:text-gray-300">
              ...
              <span v-if="item.note" class="block text-xs text-gray-500 dark:text-gray-400 italic">
                Note: {{ item.note }}
              </span>
            </span>
            <span class="font-medium whitespace-nowrap text-pif-black dark:text-white">
      ...
      <div class="space-y-4">
        ...
        <NuxtLink
          to="/products"
          class="block w-full py-3 border border-pif-green-dark text-pif-green-dark dark:text-pif-gold dark:border-pif-gold font-medium rounded-lg hover:bg-pif-green-dark hover:text-white dark:hover:bg-pif-gold dark:hover:text-pif-black transition-colors"
        >
          Continue Shopping
        </NuxtLink>
      </div>
